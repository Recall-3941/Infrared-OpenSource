package Recall.Clickgui.New.element

import com.mojang.realmsclient.gui.ChatFormatting
import net.ccbluex.liquidbounce.LiquidBounce
import net.ccbluex.liquidbounce.features.module.ModuleCategory
import Recall.Clickgui.New.ColorManager
import Recall.Clickgui.New.element.module.ModuleElement
...
class CategoryElement(val category: ModuleCategory): MinecraftInstance() {
    ...
    fun drawLabel(mouseX: Int, mouseY: Int, x: Float, y: Float, width: Float, height: Float) {
        if (focused)
            RenderUtils.drawRoundedRect(x + 3F, y + 3F, x + width - 3F, y + height - 3F, 3F, ColorManager.dropDown.rgb)
        else if (MouseUtils.mouseWithinBounds(mouseX, mouseY, x, y, x + width, y + height))
            RenderUtils.drawRoundedRect(x + 3F, y + 3F, x + width - 3F, y + height - 3F, 3F, ColorManager.border.rgb)
        Fonts.font40.drawString(name, x + 10F, y + height / 2F - Fonts.font40.fontHeight / 2F + 2F, -1)
    }
    ...
    fun drawPanel(mX: Int, mY: Int, x: Float, y: Float, width: Float, height: Float, wheel: Int, accentColor: Color) {
        ...
        Fonts.font35.drawString("${ChatFormatting.GRAY}Modules > ${ChatFormatting.RESET}$name", x + 10F, y + 10F, -1)
        Fonts.font20.drawString("$name", x - 190F, y - 12F, -1)
    ...
    private fun drawScroll(x: Float, y: Float, width: Float, height: Float) {
        if (lastHeight > height - 60F) {
            ...
            RenderUtils.drawRoundedRect(x + width - 6F, y + 5F + multiply, x + width - 4F, y + 5F + (height - 60F) * ((height - 60F) / lastHeight) + multiply, 1F, 0x50FFFFFF)
        }
    }
