...
class Slowly(inst: LBplusTarget): TargetStyle("Slowly", inst, true) {
    ...
    override fun drawTarget(entity: IEntityLivingBase) {
        ...
        font.drawStringWithShadow(healthString,
            (length + 31F - font.getStringWidth(healthString).toFloat()).toInt(), 22, targetInstance.barColor.rgb)
    ...
    override fun handleBlur(player: IEntityPlayer) {
        val font = Fonts.minecraftFont
        val healthString = "${decimalFormat2.format(player.health)} ❤"
        val length = 60.coerceAtLeast(font.getStringWidth(player.name!!)).coerceAtLeast(font.getStringWidth(healthString)).toFloat() + 10F
    ...
    override fun handleShadow(player: IEntityPlayer) {
        ...
        RenderUtils.drawRect(0F, 0F, 32F + length, 36F, shadowOpaque.rgb)
    }
