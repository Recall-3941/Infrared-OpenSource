 ...
 */
package Recall.Ui.targets.utils
...
class CharRenderer(val small: Boolean): MinecraftInstance() {
    ...
    fun renderChar(number: Float, orgX: Float, orgY: Float, initX: Float, initY: Float, scaleX: Float, scaleY: Float, shadow: Boolean, fontSpeed: Float, color: Int): Float {
        ...
        for (char in reFormat.toCharArray()) {
            moveX[indexX] = Recall.Utils.AnimationUtils.animate(animX, moveX[indexX], fontSpeed * 0.025F * delta)
            animX = moveX[indexX]
            ...
            if (pos >= 0) {
                moveY[indexY] = Recall.Utils.AnimationUtils.animate(expectAnim, moveY[indexY], fontSpeed * 0.02F * delta)
