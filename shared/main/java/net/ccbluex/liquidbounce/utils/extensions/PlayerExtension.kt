...
val IEntityLivingBase.renderHurtTime: Float
    get() = this.hurtTime - if(this.hurtTime!=0) { MinecraftInstance.mc.timer.renderPartialTicks } else { 0f }
...
val IEntityLivingBase.skin: IResourceLocation // TODO: add special skin for mobs
    get() = if(MinecraftInstance.classProvider.isEntityPlayer(this)) { MinecraftInstance.mc.netHandler.getPlayerInfo(this.uniqueID)?.locationSkin } else { null } ?: MinecraftInstance.functions.getDefaultSkinLegacy()

val IEntityLivingBase.ping: Int
    get() = if(MinecraftInstance.classProvider.isEntityPlayer(this)) { MinecraftInstance.mc.netHandler.getPlayerInfo(this.uniqueID)?.responseTime?.coerceAtLeast(0) } else { null } ?: -1
