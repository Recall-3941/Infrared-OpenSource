...
import me.aquavit.liquidsense.utils.RenderUtils.drawRoundedCornerRect
...
class FollowTargetHud : Module() {
    ...
    private fun renderNameTag(entity: IEntityLivingBase, tag: String) {
        ...
        when (modeValue.get().toLowerCase()) {
            ...
            "material" -> {
                val hudIcon = classProvider.createResourceLocation("liquidbounce" + "/custom_hud_icon.png")
                glScalef(-scale * 2, -scale * 2, scale * 2)

                // render bg
                drawRoundedCornerRect(-40f + xChange, 0f, 40f + xChange, 30f, 5f, Color(72, 72, 72, 220).rgb)
                drawImage(hudIcon,xChange.toInt(),0,5,5)
                // draw health bars
            ...
            "arris" -> {
                ...
                // info text
                font.drawString(entity.name.toString(), 40 + xChange.toInt(), 5, Color.WHITE.rgb)
                "$hp hp".also {
