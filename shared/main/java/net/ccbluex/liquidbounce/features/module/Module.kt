...
import net.ccbluex.liquidbounce.features.module.modules.misc.StateSound
...
open class Module : MinecraftInstance(), Listenable {
    ...
    var state = false
        set(value) {
            ...
            if (!LiquidBounce.isStarting) {
                if (value) {
                    (LiquidBounce.moduleManager.getModule(StateSound::class.java) as StateSound).playSound(true)
                    LiquidBounce.hud.addNotification(Notification("Module", "${"Enabled "}$name", NotifyType.SUCCESS))
                }else{
                    (LiquidBounce.moduleManager.getModule(StateSound::class.java) as StateSound).playSound(false)
                    LiquidBounce.hud.addNotification(Notification("Module", "${"Disabled "}$name", NotifyType.ERROR))
                }
            }
