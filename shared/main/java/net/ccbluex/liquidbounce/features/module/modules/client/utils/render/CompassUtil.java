package net.ccbluex.liquidbounce.features.module.modules.client.utils.render;
...
public class CompassUtil extends MinecraftInstance {
    ...
    public void draw(ScaledResolution sr) {
        ...
        int markers = 0;
        float offset = 0;
        float yaaahhrewindTime = (mc2.player.rotationYaw % 360) * 2 + 360 * 3;
        GL11.glPushMatrix();
        GL11.glEnable(3089);
        RenderUtils.doGlScissor(sr.getScaledWidth() / 2 - 120, 25, 220, 25);
        for (Degree d : degrees) {
            float location = center + (count * 30) - yaaahhrewindTime;
            float completeLocation = (float) ((location - Fonts.fontSFUI35.getStringWidth(d.text) / 2));

            int opacity = opacity(sr, completeLocation);

            if (d.type == 1 && opacity != 16777215) {
                GlStateManager.color(1, 1, 1, 1);
                Fonts.fontSFUI35.drawString(d.text, completeLocation, -75 + 100, opacity(sr, completeLocation), false);
                cardinals++;
            }
            ...
            if (d.type == 2 && opacity != 16777215) {
                ...
                GlStateManager.color(1, 1, 1, 1);
                Fonts.fontSFUI35.drawString(d.text, completeLocation, -75 + 105 + 3.5f + 4, opacity(sr, completeLocation),
                        false);
                markers++;
            }
            ...
            if (d.type == 3 && opacity != 16777215) {
                GlStateManager.color(1, 1, 1, 1);
                Fonts.fontSFUI35.drawString(d.text, completeLocation,
                        -75 + 100 + Fonts.fontSFUI35.getFontHeight() / 2 - Fonts.fontSFUI35.getFontHeight() / 2,
        ...
        for (Degree d : degrees) {
            ...
            float location = center + (count * 30) - yaaahhrewindTime;
            float completeLocation = (float) (d.type == 1 ? (location - Fonts.fontSFUI35.getStringWidth(d.text) / 2)
                    : d.type == 2 ? (location - Fonts.fontSFUI35.getStringWidth(d.text) / 2)
                    : (location - Fonts.fontSFUI35.getStringWidth(d.text) / 2));
