...
class KillAura : Module() {
    ...
    // AutoBlock
    val autoBlockValue = ListValue("AutoBlock", arrayOf("Off","Range", "Packet", "AfterTick", "Right", "Old1.8.9"), "Packet")
    ...
    // Visuals
    private val markValue = ListValue("Mark", arrayOf("New","Liquid","FDP","Block","Jello", "Plat", "Red", "Sims", "None"),"FDP")
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        if (autoBlockValue.get().equals("Range", true)) {
            if (aura.target != null) {
    ...
    fun onRender3D(event: Render3DEvent) {
        ...
        if (this.markValue.get().toLowerCase().equals("new") && !targetModeValue.get().equals("Multi", ignoreCase = true))
    ...
    fun onPacket(event: PacketEvent){
        ...
        if(classProvider.isCPacketTryUseItem(packet) && classProvider.isCPacketUseEntity(packet)){
            event.cancelEvent()
            times += 1
            ClientUtils.displayChatMessage("[Fixer]UseitemPacket has been cancelled $times times")
        }
    ...
    override val tag: String?
        get() =
            if (targetModeValue.get().contains("Switch", true))
                "SwitchTargets"
            else
                "Ticks HitTarget"
            //rotations.get()+" " +rangeValue.get()
