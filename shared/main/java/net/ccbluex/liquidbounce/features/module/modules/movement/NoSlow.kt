...
class NoSlow : Module() {
    ...
    private val modeValue = ListValue("PacketMode", arrayOf("None",  "GrimAC", "HuaYuTing",     "Vanilla","NoPacket","AAC","AAC5", "Matrix", "Vulcan", "Custom","Grim"), "AntiCheat")
        ...
        if((modeValue.equals("Matrix") || modeValue.equals("Vulcan") || modeValue.equals("GrimAC")) && (lastBlockingStat || isBlocking)) {
            if(msTimer.hasTimePassed(230) && nextTemp) {
                ...
                if(modeValue.equals("GrimAC")) {
                    PacketUtils2.sendPacketNoEvent(CPacketHeldItemChange((mc2.player.inventory.currentItem + 1) % 9))
                    PacketUtils2.sendPacketNoEvent(CPacketHeldItemChange(mc2.player.inventory.currentItem))
                } else {
                    PacketUtils2.sendPacketNoEvent(CPacketPlayerDigging(CPacketPlayerDigging.Action.RELEASE_USE_ITEM, BlockPos(-1, -1, -1), EnumFacing.DOWN))
                }
