...
@ModuleInfo(name = "LBPlusScaffold", description = "Sk1d by 2997570499.", category = ModuleCategory.WORLD, keyBind = Keyboard.KEY_I)
public class LBPlusScaffold extends Module {
    ...
    private final FloatValue AirRotSetValue = new FloatValue("AirRotationSet", 0F, 0F, 1000F);
    ...
    private final MSTimer airTimer = new MSTimer();
    private long delay;
    ...
    public void onUpdate(final UpdateEvent event) {
        air++;
        if((!Objects.requireNonNull(mc.getThePlayer()).getOnGround())&&air > AirRotSetValue.get()){
        ...
        if (sprintModeValue.get().equalsIgnoreCase("PlaceOff")) {
            ...
            mc.getThePlayer().setMotionX(mc.getThePlayer().getMotionX());
            mc.getThePlayer().setMotionZ(mc.getThePlayer().getMotionZ());
        }
    ...
    private boolean shouldPlace() {
        ...
        return towerActivation() || alwaysPlace || (placeWhenAir && Ground) || (placeWhenFall && mc.getThePlayer().getFallDistance() > 0) || (placeWhenNegativeMotion && mc.getThePlayer().getMotionY() < 0);
    }
...
//        }
    ...
    private void findBlock(final boolean expand) {
        ...
        if (expand) {
            double yaw = Math.toRadians(mc.getThePlayer().getRotationYaw() + 180);
                    ...
                    for (double zSearch = 0.5D - (xzRV / 2); zSearch <= 0.5D + (xzRV / 2); zSearch += xzSSV) {
                        ...
                        for (int i = 0; i < (staticYawMode ? 2 : 1); i++) {
                            ...
                            if (rotationModeValue.get().equalsIgnoreCase("static") && (keepRotOnJumpValue.get() || !mc.getGameSettings().getKeyBindJump().isKeyDown()))
                                rotation = new Rotation(MovementUtils.INSTANCE.getScaffoldRotation(mc.getThePlayer().getRotationYaw(), mc.getThePlayer().getMoveForward()), staticPitchValue.get());
