...
class RenderItemImpl(val wrapped: RenderItem) : IRenderItem {
    ...
    override fun renderItemAndEffectIntoGUI2(stack: ItemStack, x: Int, y: Int) = wrapped.renderItemAndEffectIntoGUI(
            stack.also { unwrap() }, x, y)

    override fun renderItemOverlays2(fontRenderer: IFontRenderer, stack: ItemStack, x: Int, y: Int) = wrapped.renderItemOverlays(fontRenderer.unwrap(),
            stack.also { unwrap() }, x, y)
